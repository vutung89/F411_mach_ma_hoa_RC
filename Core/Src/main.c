...
/**
 * @brief Function to Set Failsafe Status in SBUS Buffer
 * @param sbus_buf Pointer to the SBUS buffer
 * @param failsafe_status The failsafe status to set
 * @return void
 */
void SetSbusFailsafeStatus(uint8_t *sbus_buf, uint16_t failsafe_status)
{
	// Xóa các bit failsafe trước
	sbus_buf[23] &= ~((1 << 2) | (1 << 3));

	switch (failsafe_status) {
	case SBUS_SIGNAL_LOST:
		sbus_buf[23] |= (1 << 2);  // Signal Lost
		break;
	case SBUS_SIGNAL_FAILSAFE:
		sbus_buf[23] |= (1 << 3);  // Failsafe
		break;
	default:
		// SBUS_SIGNAL_OK: không cần set bit
		break;
	}
}
